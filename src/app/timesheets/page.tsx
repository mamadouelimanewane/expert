...
export default function TimeTrackingPage() {
    ...
    return (
        <div className="space-y-6">
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                <div>
                    <h2 className="text-3xl font-bold text-white tracking-tight flex items-center gap-3">
                        ...
                        Saisie des Temps (Timesheets)
                    </h2>
                    <p className="text-slate-400 mt-1">Suivi de la productivité et rentabilité des missions client.</p>
                </div>
                <div className="flex gap-3">
                    <div className="bg-slate-900 border border-slate-700 rounded-xl px-4 py-2 flex items-center gap-4">
                        <span className="text-xs font-bold text-slate-500 uppercase">Ce mois :</span>
                        <span className="text-white font-bold">142h <span className="text-indigo-400">/ 160h</span></span>
                    </div>
                </div>
            </div>
            ...
            {/* Active Timer Bar */}
            <div className={cn(
                "glass-card p-6 rounded-2xl border transition-all duration-500",
                ...
                <div className="flex flex-col md:flex-row items-center gap-6">
                    <div className="flex-1 w-full grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                        ...
                        <div className="flex items-center justify-center md:justify-end gap-6 pt-4 md:pt-0">
                            <div className="text-3xl font-mono font-bold text-white tracking-widest">{timer}</div>
                        </div>
                    </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 text-white">
                ...
                <div className="lg:col-span-2 space-y-4">
                    ...
                    <div className="glass-card rounded-2xl border border-slate-700/50 overflow-hidden divide-y divide-slate-800/50">
                        {MOCK_TIME_ENTRIES.map((entry) => (
                            <div key={entry.id} className="p-4 flex items-center justify-between hover:bg-slate-800/30 transition-colors group">
                                <div className="flex items-center gap-4">
                                    ...
                                    <div>
                                        <p className="font-bold text-slate-100">{entry.client}</p>
                                        <p className="text-xs text-slate-500">{entry.mission} • {entry.category}</p>
                                    </div>
                                </div>
                                <div className="flex items-center gap-8">
                                    <div className="text-right">
                                        <p className="font-mono font-bold text-white text-lg">{entry.duration}</p>
                ...
                {/* Profitability Stats */}
                <div className="glass-card rounded-2xl p-6 border border-slate-700/50 space-y-8 bg-gradient-to-br from-slate-900 to-indigo-900/10">
                    <div>
