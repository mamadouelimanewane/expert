        ...
        <div className="h-[calc(100vh-8rem)] flex gap-6">
            ...
            <div className="w-96 flex flex-col gap-6">
                ...
                <div className="glass-card rounded-2xl p-6 border border-slate-700/50">
                    ...
                    <div className="grid grid-cols-7 gap-2 text-center text-sm">
                        {["L", "M", "M", "J", "V", "S", "D"].map(d => <span key={d} className="text-slate-500 text-xs">{d}</span>)}
                        {/* Mock Calendar Grid */}
                        {Array.from({ length: 31 }, (_, i) => (
                            <div key={i} className={cn(
                                "p-2 rounded-lg cursor-pointer hover:bg-slate-800 transition-colors relative",
            ...
            <div className="flex-1 flex flex-col gap-6">
                {selectedMeeting ? (
                    <>
                        {/* Header Card */}
                        <div className="glass-card rounded-2xl p-6 border border-slate-700/50 flex justify-between items-center relative overflow-hidden">
                            {/* Background Glow */}
                            ...
                            <div className="flex gap-3 relative z-10">
                                {selectedMeeting.type === "virtuelle" && (
                                    <button className="px-5 py-2.5 bg-purple-600 hover:bg-purple-500 text-white rounded-xl font-bold transition-all shadow-lg shadow-purple-500/25 flex items-center gap-2 animate-pulse">
                            ...
                            <div className="p-4 border-b border-slate-700/50 bg-slate-900/50 flex justify-between items-center">
                                ...
                                <div className="flex gap-2">
                                    <button
                                        ...
                                        className="px-3 py-1.5 bg-gradient-to-r from-emerald-600 to-teal-600 text-white text-xs font-bold rounded-lg flex items-center gap-2 hover:shadow-lg transition-all"
                                    >
                            ...
                            <div className="flex-1 p-6 bg-slate-900/30">
                                <div className="h-full flex flex-col gap-4">
                                    <div className="flex-1 relative group">
                                        ...
                                        <textarea
                                            className="w-full h-full bg-slate-800/50 border border-slate-700 rounded-xl p-6 text-slate-300 leading-relaxed focus:outline-none focus:border-indigo-500/50 transition-all resize-none font-mono text-sm"
                                            placeholder={reportMode ? "Le rapport généré apparaîtra ici..." : "- Point 1...\n- Point 2..."}
                                            value={notes}
                                            onChange={(e) => setNotes(e.target.value)}
                                        />
                                        <div className="absolute bottom-4 right-4 text-xs text-slate-500 flex gap-2">
                                    ...
                                    <div className="flex justify-end gap-3 pt-2">
                                        <button className="px-4 py-2 bg-slate-800 text-slate-300 hover:text-white rounded-lg text-sm font-medium border border-slate-700 flex items-center gap-2">
                                            <FileText className="w-4 h-4" />
                                            Exporter PDF
                                        </button>
                                        <button className="px-4 py-2 bg-indigo-600 hover:bg-indigo-500 text-white rounded-lg text-sm font-medium shadow-lg shadow-indigo-500/20 flex items-center gap-2">
