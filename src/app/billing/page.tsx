                                ...
                                {invoices.map((inv) => (
                                    <tr key={inv.id} className="hover:bg-slate-800/20 group transition-colors">
                                        <td className="px-6 py-4">
                                            ...
                                            <p className="text-[10px] text-slate-500 uppercase">{new Date(inv.issueDate).toLocaleDateString()}</p>
                                        </td>
                                        <td className="px-6 py-4">
                                            <div className="flex items-center gap-2">
                                                <span className="font-medium text-white">{inv.client?.companyName || "N/A"}</span>
                                            </div>
                                            <span className="text-[10px] px-1.5 py-0.5 bg-slate-800 rounded text-slate-400 border border-slate-700">HONORAIRES</span>
                                        </td>
                                            ...
                                            <span className={cn(
                                                ...
                                                inv.status === "PAID" ? "bg-emerald-500/10 text-emerald-400 border-emerald-500/20" :
                                                    inv.status === "OVERDUE" ? "bg-rose-500/10 text-rose-400 border-rose-500/20" :
                                                        inv.status === "SENT" ? "bg-amber-500/10 text-amber-400 border-amber-500/20" :
                                        ...
                                        <td className="px-6 py-4 text-right">
                                            <div className="flex justify-end gap-1">
                                                ...
                                                {inv.status !== "PAID" && inv.status !== "DRAFT" && (
                                                    <button
                                                        onClick={() => setSelectedInvoice({ id: inv.id, amount: inv.total.toLocaleString() })}
