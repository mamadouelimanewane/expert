...
export default function DocumentsPage() {
    ...
    return (
        <div className="space-y-6">
            {/* Header */}
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 bg-slate-900/40 p-6 sm:p-8 rounded-3xl border border-white/5">
                <div>
                    <h2 className="text-2xl sm:text-3xl font-extrabold text-white tracking-tight flex items-center gap-3">
                        GED Intelligente
                        <span className="hidden sm:inline text-[10px] bg-indigo-600 px-2 py-0.5 rounded-full uppercase font-black">Powered by IA</span>
                    </h2>
                    <p className="text-slate-400 mt-1 text-sm sm:text-base leading-relaxed">Classification automatique et indexation sémantique (ISO 27001).</p>
                </div>
                <div className="flex flex-col sm:flex-row gap-3 w-full sm:w-auto">
                    <button className="w-full sm:w-auto px-5 py-3 bg-slate-800 hover:bg-slate-700 text-white rounded-xl font-bold transition-all border border-white/5 flex items-center justify-center gap-2">
                        <Folder className="w-4 h-4" />
                        <span className="sm:hidden">Dossiers</span>
                        <span className="hidden sm:inline">Structuration Dossier</span>
                    </button>
                    <button
                        onClick={handleUpload}
                        className="w-full sm:w-auto px-5 py-3 bg-indigo-600 hover:bg-indigo-500 text-white rounded-xl font-bold transition-all shadow-xl shadow-indigo-600/20 flex items-center justify-center gap-2 active:scale-95"
                    >
            ...
            {isUploading ? (
                ...
                <div className="glass-card rounded-[32px] overflow-hidden border border-white/5 bg-slate-900/20">
                        ...
                        <ResponsiveDataList
                            ...
                            renderCard={(file) => (
                                <div 
                                    ...
                                    className="bg-white/5 p-4 rounded-2xl border border-white/5 flex items-center justify-between group active:scale-[0.98] transition-transform"
                                >
                                    <div className="flex items-center gap-4">
                                        ...
                                        <div className="min-w-0">
                                            ...
                                            <div className="flex items-center gap-2 mt-0.5">
                                                <span className="text-[9px] px-1.5 py-0.5 bg-slate-800 rounded-full text-slate-500 font-bold uppercase">{file.category}</span>
                                                <span className="text-[9px] text-slate-600 font-black uppercase text-xs">{file.confidence}% Conf.</span>
                                            </div>
                                        </div>
                                    </div>
